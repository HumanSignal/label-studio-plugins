...
function tooSimilar(deviation = 0.1, max_count = 10) {
  return (items, field) => {
    ...
    return calcDeviation(points) < deviation ? MESSAGES.tooSimilar({ items, field }) : false;
  };
}
...
function tooFast(minutes = 10, times = 20) {
  return (items) => {
    ...
    return last.created_at - first.created_at < minutes * 60 ? MESSAGES.tooFast({ items }) : false;
  };
}
...
const values = Object.fromEntries(fields.map((field) => [field, DM.project.parsed_label_config[field]?.labels]));
